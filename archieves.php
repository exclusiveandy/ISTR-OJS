...
$journal_query = query("SELECT * from journal_table where journal_id = '{$journal_id}'");
  ...
  <div class="content-wrapper">
    ...
    <div class="content">
      ...
      <div class="row">
       ...
       <?php 
       $volume_query = query("SELECT volume, volume_id, issue, cover_page, DATE_FORMAT(date_published, \"%Y\") as year from volume_table v where journal_id = '{$journal_id}' AND v.status = 
        '1' order by date_published desc");
       while($row_volume = fetch_assoc($volume_query))
       {
        ?>
              <ul class="timeline">
              <li  class="time-label">
                <span class=""><?php echo $row_volume['year'];?> </span>            <!-- /.GROUPED BY YEAR AND MONTH -->
              </li>
