...
if(!empty($_POST))
{
 ...
 <h3 class="card-title">List of Authors <strong>('.$row_count['count'].')</strong></h3>
   ...
   <div class="card-body">';
    $output .= '<label class= "text-sucess">'.$message.'</label>';
      $sql = query("SELECT CONCAT(user_fname, \" \", user_mname,  \" \" ,user_lname) as FULLNAME, user_email, user_id, user_affliation, DATE_FORMAT(register_date, \"%M %d %Y %r\") as register_date, gender, expertise, user_role_name from user_table u1 join user_role_table u2 on u1.user_role_id = u2.user_role_id where u2.user_role_name = \"Author\" and activate = 1 order by register_date desc");
    $output .= '<table id="example1" class="table table-bordered table-striped">
              <thead>
            <tr>
              <th>Name</th>
