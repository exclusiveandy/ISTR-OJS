...
if(isset($_GET['r_id']) && isset($_GET['remarks']))
{
	...
	$sql = query("UPDATE research_table SET status = 'To the Proofreader', user_role_id = '3' where research_id = '{$id}' ");
    $chk_history_query = query("SELECT * from research_file_history_table where research_id = '{$id}' AND user_role_id = '5'");
    ...
    $insert_timeline = query("INSERT INTO timeline_table(document_id, Type, Remarks, timeline_date) VALUES ('{$id}', 'Accepted', 'Accepted the Document by the External Reviewer', '{$date}')");
    }
    else
    {
   ...
   // send_email($email_eic, $subject2, $msg2);
    ...
    $insert_into = query("INSERT INTO notification(status, message, date, user_id, notification_type, research_id)
                   VALUES('unread', '{$message}', '{$date}', '{$user_eic}', 'for_PO', '{$id}')");
     redirect("home.php");
}
?> 