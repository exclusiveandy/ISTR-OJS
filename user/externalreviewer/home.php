...
?>
  ...
  $count_review_query = query("SELECT  COUNT(r1.research_id) as count
     ...
     where r2.user_id =  '{$user}' and r1.user_role_id = 5");
    ...
    <label>As External Reviewer</label>
            <!-- small card -->
    ...
    $count_ongoing_query = query("SELECT COUNT(research_id) as count
     FROM `research_table`as r 
     where user_id = '{$user}' and status NOT IN  (\"Accepted with Revisions\", \"To the Author for Consent\", \"Rejected\") order by research_id asc");
