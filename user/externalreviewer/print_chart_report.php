...
having MAX(research_file_history_table.user_role_id)+1 <> 5))
 GROUP BY month_table.month
ORDER BY month_table.id ASC");
...
AND reviewer_table.user_id = '{$user_id}'
group by research_file_history_table.research_id
having MAX(research_file_history_table.user_role_id)+1 = 5) and research_table.status = \"Rejected\")
 GROUP BY month_table.month
ORDER BY month_table.id ASC");
...
$pdf->Ln();
$pdf->Cell(110,10,'Role: External Reviewer',0, 0);
$pdf->Ln();
$pdf->headertable();
$pdf->passed_table();
$pdf->AddPage('L','A4',0);
...
$pdf->Ln();
$pdf->SetFont('Times','B',15);
$pdf->Cell(110,10,'As External Reviewer',0, 0);
$pdf->Ln();
$pdf->headertable2();
$pdf->passed_table2();
$pdf->AddPage('L','A4',0);
