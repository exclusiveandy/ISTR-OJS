...
if(isset($_GET['r_id']))
{
	...
	$sql = query("UPDATE research_table SET status = 'On Layout Editor', user_role_id = '7' where research_id = '{$id}' ");
      ...
      $date =  date('Y-m-d H:i:s',strtotime('+1 sec',strtotime($date)));
         $insert_timeline1 = query("INSERT INTO timeline_table(document_id, Type, Remarks, timeline_date) VALUES ('{$id}', 'Reviewing', 'Send to the Publication Office for assigning a layout editor', '{$date}')");
   ...
   $comment_query = query("SELECT * from comment_table where research_id = '{$id}'");
    ...
    $msg = "The Document that you passed on Online Journal System(ISTR) has been updated <a href=\"http://localhost/OJS/istr/user/author/view_document.php?r_id=$id\">Link Here</a>";
     ...
     $email_query = query("SELECT user_email as email from user_table u1 where u1.user_role_id = 7 LIMIT 1");
     confirm($email_query);
