<?php include("usernav.php");?>
 ...
 <?php
     ...
     <?php
      ...
      <?php
    $query = query("SELECT u1.user_id, CONCAT(user_fname, \" \", user_mname, \" \", user_lname) as fullname, Expertise from user_table u1 where user_role_id = 7");
    confirm($query);
...
?>
      ...
      WHERE research_id = '{$_GET['r_id']}' and u2.user_role_name = \"Layout Editor\" ");
      if(row_count($chk_proofreader) == 0)
      {
        ?>
      <div class="modal-header">
              <h4 class="modal-title">Assign a Layout Editor</h4>
            ...
            <div class="modal-body">
                  ...
                  <div class="form-group">
                    <label>Layout Editors</label>

                    <select class="form-control" name="USER">
                    <?php 
                        $query = query("SELECT  u1.user_id, CONCAT(user_fname, \" \", user_mname, \" \", user_lname) as fullname from user_table u1 where user_role_id = 7");
                        confirm($query);
      ...
      ?>
      <div class="modal-header">
              <h4 class="modal-title">Send to the Assigned Layout Editor</h4>
            ...
            <div class="modal-body">
                  ...
                  <div class="form-group">
                    ...
                    <select class="form-control" name="USER">
                  <?php 
                        $query = query("SELECT u1.user_id, CONCAT(user_fname, \" \", user_mname, \" \", user_lname) as fullname, Expertise FROM layouteditor_table r 
      join user_table u1 
      on r.user_id=u1.user_id 
      join user_role_table u2
      on u1.user_role_id=u2.user_role_id
    ...
    else
    {
      ?>
           <div class="modal-header">
              <h4 class="modal-title">There is no currently Layout Editor for the system. <br> Kindly contact the admin</h4>
         </div>
     <?php
     }
     ?> 
     ...
     $chk_external_reviewer =  query("SELECT * FROM layouteditor_table r 
      join user_table u1 
      on r.user_id=u1.user_id 
      join user_role_table u2
      on u1.user_role_id=u2.user_role_id
...
$date_expired = date("Y-m-d H:i:s", strtotime($date. ' + 3 days'));
          $date_expired;
          $sql =  query("INSERT INTO layouteditor_table(user_id, research_id, date_uploaded, date_expired) VALUES ('{$u_id}','{$r_id}', '{$date}', '{$date_expired}')");      }
      ...
      if(!empty($_POST['Comment']))
      {
        ...
        $insert_timeline = query("INSERT INTO timeline_table(document_id, Type, Remarks, timeline_date) VALUES ('{$r_id}', 'Reviewing', 'Send to the Layout Editor', '{$date}')");      
    $insert_timeline2 = query("INSERT INTO timeline_table(document_id, Type, Remarks, timeline_date) VALUES ('{$r_id}', 'Reviewing', 'Being Processed by the Layout Editor', '{$date}')");        
     ...
     $user_id = $row['user_id'];
      ...
      $message = "There is an article entitled:".$title." that you need to do the layout";
      $insert_into = query("INSERT INTO notification(status, message, date, user_id, notification_type, research_id)
                    VALUES('unread', '{$message}', '{$date}', '{$user_id}', 'review_layout', '{$r_id}')");
